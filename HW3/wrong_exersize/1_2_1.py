...
def find_piece(img1, img2):
    ...
    for i in range(rows2-rows):
        for j in range(5,cols//2):
            ...
            if cor>max_cor[0]:
                max_cor[0] = cor
                cor_place[0] = (i, cols//2+j, 0)
    for i in range(rows1-rows):#same code
        for j in range(5, cols//2):
            img1_cut = img1[i:rows+i,:j]
            img2_cut = img2[:, (cols//2)+j:cols2]
            hist1 = cv2.calcHist([img1_cut.ravel()], [0], None, [256], [0,256])
            hist2 = cv2.calcHist([img2_cut.ravel()], [0], None, [256], [0,256])
            hist1 /= hist1.max()
            hist2 /= hist2.max()
...
def connect_images(img1, img2, place):
    ...
    if place[2] == 0:
        ...
        result = np.zeros((rows_max, place[1]+cols1, 3), dtype=np.int64)
