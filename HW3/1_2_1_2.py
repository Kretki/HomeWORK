...
def find_piece(image1, image2):
    lead = 0
    core_row1, core_col1 = image1.shape
    core_row2, core_col2 = image2.shape
    ...
    for i in range(rows1-rows+1):
        for j in range(CORR_COST,3*(cols//4)):
            ...
            if cor>max_cor[0]:
                max_cor[0] = cor
                cor_place[0] = (i, 3*(cols//4)+j-CORR_COST, lead, 0)
    ...
    for i in range(cols1-cols+1):
        for j in range(CORR_COST, 3*(rows//4)):
            ...
            if cor>max_cor[2]:
                max_cor[2] = cor
                cor_place[2] = (3*(rows//4)+j-CORR_COST, i, lead, 2)
...
def compete_images(images):
    ...
    for i in range(1, len(images)):
        ...
        place, cor = find_piece(res_bw, images_bw[i])
        if cor>0.8:
